...
const ExerciseTwo = ({ onComplete }: ExerciseTwoProps) => {
  ...
  useEffect(() => {
    ...
    sendTutorMessage('instruction', 'messages.fraction_explorer.ex2_split_one_half.intro');
    sendTutorMessage('instruction', 'messages.fraction_explorer.ex2_split_one_half.prompt_split_top');
  }, [sendTutorMessage]);
  ...
  const handleTopSplit = () => {
    ...
    setTimeout(() => {
      ...
      sendTutorMessage('instruction', 'messages.fraction_explorer.ex2_split_one_half.after_split_prompt');
    }, 1000);
  };
  ...
  const handleFractionSelect = (fraction: string) => {
    ...
    if (correct) {
      sendTutorMessage('success', 'messages.fraction_explorer.ex2_split_one_half.success_top_piece');
      setTimeout(() => {
        setShowSumTask(true);
        sendTutorMessage('instruction', 'messages.fraction_explorer.ex2_split_one_half.sum_prompt');
      }, 1500);
    } else {
      sendTutorMessage('instruction', 'messages.fraction_explorer.ex2_split_one_half.incorrect_top_piece');
    }
  };
  ...
  const handleSumSelect = (sum: string) => {
    ...
    if (correct) {
      sendTutorMessage('success', 'messages.fraction_explorer.ex2_split_one_half.sum_success');
      setTimeout(() => {
        onComplete();
      }, 1500);
    } else {
      sendTutorMessage('instruction', 'messages.fraction_explorer.ex2_split_one_half.sum_incorrect');
    }
  };
